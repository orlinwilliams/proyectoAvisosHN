...
switch ($_GET['accion']) { //DATOS DEL DIA
    ...
    case '4': // RANGO DE FECHAS
        ...
        $conexion = new Conexion();
        $datos = array();
        $sqlPublicaciones = "SELECT fechaPublicacion, CASE MONTH(fechaPublicacion)
                                WHEN 1 THEN 'Enero'
                                WHEN 2 THEN 'Febrero'
                                WHEN 3 THEN 'Marzo'
                                WHEN 4 THEN 'Abril'
                                ...
                                END mes, COUNT(*) as publicaciones
                                FROM anuncios
                                WHERE fechaPublicacion>=$fecha1 AND fechaPublicacion<=$fecha2
                                GROUP BY fechaPublicacion, mes
                                ORDER BY fechaPublicacion ASC;";
        $sqlCategoria = "SELECT nombregrupo, COUNT(*) as publicaciones
                            FROM anuncios
                            INNER JOIN categoria ON categoria.idCategoria=anuncios.idCategoria
