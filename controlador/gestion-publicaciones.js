...
mifuncionFecha = function(m){
    ...
    method: "POST",
    data: "txt_idanuncio=" +$("#fechaidanuncio"+m+"").val()+
...
tablaGestionPublicaciones = function () {
  ...
  $.ajax({
      ...
      for (let item of datos) {
        ...
        "<input type='text' id='"+i+"' onchange='mifuncionFecha("+i+")'  class='datetimepicker form-control' placeholder='"+item.fechaLimite+"' value='' >"+
        "<!--EN EL VALUE DEBE CARGAR LA HORA DE LA BASE DE DATOS-->"+
        "<input type='hidden' readonly='readonly' id='fechaidanuncio"+i+"' value='"+item.idAnuncios+"'>"+
        "<div type='hidden' readonly id='fecha2' value=''></div>"+
        "</div>"+
        "</td>"+
        "<td style=' text-align:center;''> <button type='button' class='btn bg-red waves-effect' onclick='eliminarPublicacion("+item.idAnuncios+","+item.idUsuario+")'>"+
